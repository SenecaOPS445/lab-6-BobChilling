...
class Student:
    ...
    def displayGPA(self):
        ...
        if len(self.courses) == 0:
            ...
            for course in self.courses.keys(): # Self note: For loop scan through all of self.courses and add grades to gpa 
                gpa = gpa + self.courses[course]
        return 'GPA of student ' + self.name + ' is ' + str(gpa / len(self.courses)) # Self note: Gpa divides the amount (len) of the courses for average
    
    # Gpa is 0.0, if there are no courses or grade is 0 a ZeroDivisionError occours
    # Idea: use an if statement to check if there are no courses or no grade

    # Return a list of course that the student passed (not a 0.0 grade)
    # Idea: Check script for courses that is not 0.0 then for loop print all courses that's not 0.0
